...
class OperateModelBase:
    ...
    @abstractmethod
    def param_adapter(self: any, inputs: list, outputs: list, attr: dict) -> tuple:
        """
        归一产品侧和训练侧的输入，如果本身一致可以直接返回
    ...
    def predict(self: any, inputs: list, outputs: list, attr: dict) -> tuple:
        ...
        inputs, outputs, attr = self.param_adapter(inputs, outputs, attr)
